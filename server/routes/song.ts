...
songRouter.get('/api/songs/:id/song-resources', (req, res) => {
  const songId = Number(req.params.id)
...
songRouter.post('/api/songs/:id/song-resources', (req, res) => {
  const songId = Number(req.params.id)
  const song = db.prepare('SELECT id FROM songs WHERE id=?').get(songId)
  if (!song) {
    return res.status(404).send('Morceau introuvable')
...
songRouter.get('/api/songs/:id/part-resources', (req, res) => {
  const songId = Number(req.params.id)
  const song = db.prepare('SELECT id FROM songs WHERE id=?').get(songId)
  if (!song) {
    return res.status(404).send('Morceau introuvable')
...
songRouter.post('/api/songs/:id/part-resources', (req, res) => {
  const songId = Number(req.params.id)
  const song = db.prepare('SELECT id FROM songs WHERE id=?').get(songId)
  if (!song) {
    return res.status(404).send('Morceau introuvable')
